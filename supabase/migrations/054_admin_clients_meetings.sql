-- 054 · admin_clients() expõe reuniões/transcrições (Tarefa D3/D5)
-- No card do funil basta saber SE tem reunião/transcrição e a DATA da última (não o conteúdo).
-- Corpo idêntico ao da 050 + (meetings_count, last_meeting_at, has_transcript) de delivery.client_meetings.
...
begin
  ...
  return (select coalesce(json_agg(t order by t.mrr desc, t.name), '[]'::json) from (
    select o.id, o.name, o.plan, o.stage, o.country, o.tax_id, o.website, o.founded_on, o.owner_name,
      ...
      o.is_donation, o.donation_value, o.donated_at, o.contract_status, o.segmento, o.uf, o.is_ngo,
      ...
      (select count(*) from delivery.client_meetings m where m.organization_id = o.id) as meetings_count,
      (select max(m.meeting_at) from delivery.client_meetings m where m.organization_id = o.id) as last_meeting_at,
      coalesce((select bool_or(coalesce(m.transcript,'') <> '') from delivery.client_meetings m where m.organization_id = o.id), false) as has_transcript,
