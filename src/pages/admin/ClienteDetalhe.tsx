...
// ISO (timestamptz) → "YYYY-MM-DDTHH:MM" local, p/ o input datetime-local da entrega real.
...
export default function ClienteDetalhe({ onStageChange }: { onStageChange?: (s: string) => void } = {}) {
  ...
  const [rolloutForm, setRolloutForm] = useState<Record<string, { label: string; blurb: string; progress: string; start: string; due: string; delivered: string; status: string; monthly: string; setup: string; contract: string; cost_allocation: string }>>({});
  ...
  useEffect(() => {
    ...
    const rf: Record<string, { label: string; blurb: string; progress: string; start: string; due: string; delivered: string; status: string; monthly: string; setup: string; contract: string; cost_allocation: string }> = {};
    cms.forEach((c) => { rf[c.id] = { label: c.label ?? "", blurb: c.blurb ?? "", progress: String(c.rollout_progress ?? 0), start: c.rollout_start ?? "", due: c.rollout_due ?? "", delivered: dtLocal(c.delivered_at), status: c.rollout_status ?? "in_progress", monthly: c.monthly_cost != null ? String(c.monthly_cost) : "", setup: c.setup_cost != null ? String(c.setup_cost) : "", contract: c.contract_date ?? "", cost_allocation: c.cost_allocation ?? "" }; });
    setRolloutForm(rf);
  ...
  async function saveRollout(cmId: string) {
    ...
    try {
      ...
      await api.delivery.clientModules.updateRollout(cmId, { label: rf.label.trim() || null, blurb: rf.blurb.trim() || null, rollout_progress: Math.max(0, Math.min(100, Number(rf.progress) || 0)), rollout_start: rf.start || null, rollout_due: rf.due || null, delivered_at: rf.delivered ? new Date(rf.delivered).toISOString() : null, rollout_status: rf.status, monthly_cost: num(rf.monthly), setup_cost: num(rf.setup), contract_date: rf.contract || null, cost_allocation: rf.cost_allocation || null });
      reload(); flash(tr("Instância salva ✓"));
    }
    catch (e) { flash(tr("Erro:") + " " + errorMessage(e)); } finally { setBusy(false); }
  }
  const setRf = (cmId: string, patch: Partial<{ label: string; blurb: string; progress: string; start: string; due: string; delivered: string; status: string; monthly: string; setup: string; contract: string; cost_allocation: string }>) =>
  ...
  return (
    <div>
      ...
      {cm.length === 0 ? <Empty>{tr("Nenhum módulo contratado — libere módulos acima para acompanhar a implantação.")}</Empty> : cm.map((c: any) => {
        ...
        const rf = rolloutForm[c.id] || { label: "", blurb: "", progress: "0", start: "", due: "", delivered: "", status: "in_progress", monthly: "", setup: "", contract: "" };
        return (
          <div className="rollrow" key={c.id}>
            ...
            <label className="rollf"><span>{tr("Início")}</span><input type="date" value={rf.start} onChange={(e) => setRf(c.id, { start: e.target.value })} /></label>
            ...
            <label className="rollf" title={tr("Quando a solução ficou pronta (data e hora)")}><span>{tr("Entrega real")}</span><input type="datetime-local" value={rf.delivered} onChange={(e) => setRf(c.id, { delivered: e.target.value })} /></label>
