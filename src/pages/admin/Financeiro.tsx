...
export default function Financeiro() {
  ...
  const { data, loading, reload } = useAsync(async () => {
    const [pay, rec, costs] = await Promise.all([
      services.finance.accounts.list("payable"), services.finance.accounts.list("receivable"), services.finance.costs.list(),
    ]);
    return { pay: (pay as any[]) ?? [], rec: (rec as any[]) ?? [], costs: (costs as any[]) ?? [] };
  }, []);
  const pay = data?.pay ?? [], rec = data?.rec ?? [], costs = data?.costs ?? [];
  ...
  return (
    <div>
      ...
      <div className="kpis" style={{ marginBottom: 16 }}>
        ...
        <div className="kpi"><div className="lab">{t("Saldo em Caixa")}</div><div className="val tnum" style={{ fontSize: 22 }}>{money(0)}</div><div className="delta">{t("via Tesouraria (em breve)")}</div></div>
      ...
      {loading ? <Empty>Carregando…</Empty> : !built ? (
