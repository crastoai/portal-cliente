...
// Anexa um documento (R2) — reutilizável (skill/regra/conhecimento, comprovante etc.).
// Guarda a CHAVE do R2 (path) + o nome do arquivo. prefix define a "pasta" no R2;
// accept e label permitem adequar ao contexto (ex.: comprovante = imagem/PDF).
export default function DocField({ path, name, onChange, prefix = "console", accept = ".pdf,.doc,.docx,.txt,.md,.csv,.xlsx", label }: { path?: string; name?: string; onChange: (path: string, name: string) => void; prefix?: string; accept?: string; label?: string }) {
  const t = useT();
  ...
  async function pick(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setBusy(true); setErr("");
    try { const key = await services.storage.upload(prefix, file); onChange(key, file.name); }
    ...
    <div>
      {path ? (
        ...
        <button type="button" className="crasto-btn crasto-btn--secondary crasto-btn--sm" disabled={busy} onClick={() => inp.current?.click()}>
          <span className="crasto-btn__icon"><Upload size={14} /></span><span className="crasto-btn__label">{busy ? t("Enviando…") : (label || t("Anexar documento"))}</span>
        </button>
      )}
      {err && <div className="mt" style={{ color: "var(--crasto-danger)" }}>{err}</div>}
      <input ref={inp} type="file" hidden onChange={pick} accept={accept} />
    </div>
  );
}
