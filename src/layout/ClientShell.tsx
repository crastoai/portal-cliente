...
export default function ClientShell() {
  ...
  const { data: myScreens } = useAsync(() => services.identity.access.myScreens(), []);
  ...
  const scopeKey = "crasto_active_unit_self";
  ...
  // comum só seleciona. Não há mais caso especial de "ver como cliente": entrar como cliente é
  // impersonação, então o /api/me do wacrm já responde pela org DELE. O wacrm reconfere no banco.
  ...
  // `undefined` (units carregando) = sem tag. Quando um admin entra como cliente (impersonação), a
  // sessão JÁ é a do cliente — as unidades vêm dele, sem caso especial aqui.
  const companyName = (units.find((u) => u.is_primary) || units[0])?.name || undefined;

  return (
    <UnitScopeContext.Provider value={{ units, unitId, setUnitId, canManage: canManageUnits, createUnit, reload: reloadUnits }}>
      <Shell nav={nav} bottomNav={bottomNav} brandTag={companyName} who={profile?.full_name || "Cliente"} sub="Portal do Cliente" logoTone="linear-gradient(145deg,#1F8A5B,#0d5c3a)" />
